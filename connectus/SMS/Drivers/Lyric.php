<?php
    ...
    class Lyric extends SMSAbstract
    {
        ...
        public function api_queue_sms($number, $message, $id_envio, $rest = '')
        {
            ...
            $channel    = $this->channel; # obtenemos un array con los canales disponibles
            $nch        = count($this->channel);      # numero de canales disponibles
            $ich        = 0;                    # index channel
            ...
            foreach($splitMessage as &$readyMessage){
                ...
                if ($compania != 'SIN EMPRESA') {
                    ...
                    $arrayName = $this->send_lyric($number, $channel[$ich], $readyMessage);
        ...
        private function send_lyric($number, $channel, $readyMessage)
        {
            ...
            return json_decode($json);// devuelve un objeto
        }
        ...
        public function sendFailoverMasivo($detalle, $campos, $contacto)
        {
            $mensaje = $detalle['cuerpo'];

            foreach ($campos as $key => $campo)
            {
                $search     = '%'.strtolower($campo['glosa']).'%';
                $replace    = $contacto[$campo['campo']];
                $mensaje    = str_ireplace($search, $replace, $mensaje);
            }

            $channel = $this->channel[array_rand($this->channel, 1)];

            $result = $this->send_lyric($detalle['destinatario'], $channel, $mensaje);
        ...
        public function sendFailoverIndividual($detalle, $contacto)
        {
            $detalle['id_detalle_envio']."<br>";

            $channel = $this->channel[array_rand($this->channel, 1)];
            
            $this->send_lyric($detalle['destinatario'], $channel, $detalle['cuerpo']);
