...
class SmsController{
	...
	function __construct() {
		...
		$this->clientServerSms = new SoapClient(dirname(__FILE__) .'/tempconvert.xml',array(
			'exceptions'=>true,
	...
	*/
	...
	public function sendMassTest2(){
		...
		//##############################################################
			...
			/*  Cantidad para probar resultados */
			$NUMERO_DE_ENVIOS = 100;


				for ($i = 0; $i < $NUMERO_DE_ENVIOS; $i++) {

						/*
						$params       = array( "FromCurrency" => 'USD',
												"ToCurrency" => 'CLP' );
			
						$requestIds[] = $client->ConversionRate($params);
						*/
						//$requestNumbers[end($requestIds)] = array($number, $compania);

						$params = array(
						"clientid" => 'telecotesting',
						"clientpassword" => 'matias2015',
						"ani" =>  '56442147839',
						"dnis" => '56958481218',
						"message" => utf8_encode('Mensaje a nico numero: '.$i)
						);
			...
			foreach ($responses as $id => $response) {
			    ...
			    if ($response instanceof SoapFault) {
			        ...
			        //print $mensaje;
 			       // $this->write_logCrone($arrayName['return']);
			        ...
			        $mensaje = '<br> ### Number:'.$requestNumbers[$id][0]."\n";  
	...
	public function sendMassTest(){
		...
		try {
			...
			$client = new SoapClientAsync('http://developer.assertsoft.cl/grupodm/soap/SoapServerCustom.php?wsdl',array(
									'exceptions'=>true,
									'cache_wsdl'=>WSDL_CACHE_NONE,
									'encoding'=>'utf-8'));
			...
			for ($i = 0; $i < $NUMERO_DE_ENVIOS; $i++) {
			    $params       = array( "celsius" => $i );

			    echo $i."<br>___<br>";
			    

			    echo $client->CelsiusToFahrenheit($params) . " request -> $i" ;

			    echo end($requestNumbers)."<br>";
			}
	...
	{
		$arch = fopen(DIR_LINK . 'connectus/Logs/test_log.txt', 'a');
	...
	{
		$arch = fopen('/home/connectus/public_html/' . 'connectus/Logs/test_log.txt', 'a');
		date_default_timezone_set('America/Santiago');
		fwrite($arch, date('Y-m-d G:i:s') . ' - ' . print_r('-------------------------', true) . "\n");
		fwrite($arch, date('Y-m-d G:i:s') . ' - ' . print_r($cadena, true) . "\n");
...
$testasync = new SmsController();
$testasync->sendMassTest2();
