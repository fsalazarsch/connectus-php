...
class SmsEnvioController extends SMS
{
	...
	function __construct() {
		...
		require_once "wsteleco/lib/xmlrpc.inc";

		# WebService Teleco
		$this->clientServerSms = new xmlrpc_client("http://smpp2.telecochile.cl:4080");
		...
		$this->conDB = new PDO($string_connection,'root','');				        		
	...
	*/
	...
	public function addEnvio($estado ,$remitente, $id_mensaje, $tipo_envio, $tipo_mensaje, $nombre_envio, $id_empresa = '', $id_lista = '', $id_consumo){			
   	...
   	public function addDetalleEnvio($id_respuesta_servidor, $id_envio, $destinatario, $compania, $channel){		
	...
	public function sendSMStoTeleco($number, $message, $id_envio, $rest = ''){
		...
		foreach($splitMessage as &$readyMessage){
			...
			if ($compania != 'SIN EMPRESA') {
				...
				$id_detalleEnvio = $this->addDetalleEnvio($result['ID'], $id_envio, $number, $compania, null);
				...
				$id_detalleEnvio = $this->addDetalleEnvio('0', $id_envio, $number, $compania, null);
			}
		...
		if (empty($rest)) {
			...
			# se retornaba el id devuelto por teleco, ahora retornaremos el ID del mensaje (de la tabla detalleEnvio)
			#return $id_mensaje;
