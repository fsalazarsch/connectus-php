...
class ModelSaleCustomer extends Model
{
	...
	{		
		...
		if (isset($data['password'])) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "',razon_social ='" . $this->db->escape($data['firstname']) . "',lastname = '" . $this->db->escape($data['lastname']) . "',rut_empresa='" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? serialize($data['custom_field']) : '') . "', newsletter = '" . (int)$data['newsletter'] . "', salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', status = '" . (int)$data['status'] . "', approved = '" . (int)$data['approved'] . "', safe = '" . (int)$data['safe'] . "', date_added = NOW()");
		}else{
			$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "',razon_social ='" . $this->db->escape($data['firstname']) . "',lastname = '" . $this->db->escape($data['lastname']) . "',rut_empresa='" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? serialize($data['custom_field']) : '') . "', newsletter = '" . (int)$data['newsletter'] . "', status = '" . (int)$data['status'] . "', approved = '" . (int)$data['approved'] . "', safe = '" . (int)$data['safe'] . "', date_added = NOW()");
		}

		$customer_id = $this->db->getLastId();

		$this->load->model('cuenta_corriente/cuenta_corriente');
		$this->model_cuenta_corriente_cuenta_corriente->addCuentaCorriente($customer_id);
		

		/*
		if (isset($data['address'])) {
			foreach ($data['address'] as $address) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "address SET customer_id = '" . (int)$customer_id . "', firstname = '" . $this->db->escape($address['firstname']) . "',razon_social='" . $this->db->escape($address['firstname']) . "', lastname = '" . $this->db->escape($address['lastname']) . ",lastname='" . $this->db->escape($address['lastname']) . "', company = '" . $this->db->escape($address['company']) . "', address_1 = '" . $this->db->escape($address['address_1']) . "', address_2 = '" . $this->db->escape($address['address_2']) . "', city = '" . $this->db->escape($address['city']) . "', postcode = '" . $this->db->escape($address['postcode']) . "', country_id = '" . (int)$address['country_id'] . "', zone_id = '" . (int)$address['zone_id'] . "', custom_field = '" . $this->db->escape(isset($address['custom_field']) ? serialize($address['custom_field']) : '') . "'");
		...
		if (isset($data['direccion'])) {
			...
			$sql = "	INSERT INTO " . DB_PREFIX . "address SET 
						...
						postcode = '0',
						custom_field = '',
						address_1 = '" . $this->db->escape($data['direccion']) . "'";

			# comuna
			if(isset($data['comuna']))
			{	
				$sql .= ",	address_2 = '" . $this->db->escape($data['comuna']) . "'";
			}

			# ciudad
			if(isset($data['ciudad']))
			{	
				$sql .= ",	city = '" . $this->db->escape($data['ciudad']) . "'";
			}
		}
	...
	public function get_empresa_por_user($user_id)
	{
		$sql = "SELECT id_empresa from " . DB_PREFIX . "user WHERE user_id = " . $user_id;
	...
	public function editCustomer($customer_id, $data) {
		...
		$result = $this->db->query("UPDATE " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? serialize($data['custom_field']) : '') . "', newsletter = '" . (int)$data['newsletter'] . "', status = '" . (int)$data['status'] . "', approved = '" . (int)$data['approved'] . "', safe = '" . (int)$data['safe'] . "', razon_social='" . $this->db->escape($data['firstname']) . "', rut_empresa='" . $this->db->escape($data['lastname']) . "' WHERE customer_id = '" . (int)$customer_id . "'");
	...
	public function getCustomer($customer_id) {
		$query = $this->db->query("	SELECT DISTINCT c.*, a.address_1, a.address_2, a.city
									FROM assert_customer c LEFT JOIN assert_address a ON c.customer_id = a.customer_id
	...
	public function getCustomers($data = array()) {
		...
		$sql = "SELECT *, razon_social AS name, cgd.name AS customer_group FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (c.customer_group_id = cgd.customer_group_id) WHERE cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";

		$implode = array();

		if (!empty($data['filter_name'])) {
			//$implode[] = "CONCAT(c.firstname, ' ', c.lastname) LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
			$implode[] = "razon_social LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
		}
	...
	public function getAddress($address_id) {
		...
		if ($address_query->num_rows) {
			...
			return array(
				...
				'direccion'		=> 	$address_query->row['address_1'],
				'comuna'		=>	$address_query->row['address_2'],
				'ciudad'		=>	$address_query->row['city']
			);
		}
	}
