...
/*
   ...
   public function decodMensaje($id_envio,$id_contacto,$destinatario)
    ...
    {
        $sql = "SELECT count(id_detalle_envio) as error from detalle_envio where estado in ( 'UNDELIVERED','UNKNOWN','INVALID_DNS') and id_envio = " . $id_envio;
        $result = $this->admDB->query($sql);
    ...
    public function cantidadDeEnvios($id_usuario,$tipo,$mensaje, $data = array() )
    {
        $sql = "select count(e.id_envio) as total from envio as d  inner join envio as e on d.id_envio = e.id_envio
                and e.tipo_mensaje = '" . $mensaje . "' and e.tipo_envio = '" . $tipo . "' and e.id_empresa =". $this->session->data['id_empresa'];
