...
class ModelSmsEnvio extends Model{	
    ...
    public function contarDetalle($id_envio, $estado = '')
    {
        ...
        if (!empty($estado)) {     
            if ($estado == 'confirmados') {
                ...
                $sql .= " AND estado in ('INVALID_DNS')";  
            }  elseif ($estado == 'error') {
                $sql .= " AND estado in ('UNDELIVERED', 'UNKNOWN')";  
            }   
        }
    ...
    public function desgloseEnvio($id_envio)
    {
       ...
       foreach ($empresas as $valor) {
        ...
        $sinEmpresa['malos'] += $this->smsMalos($id_envio);
    ...
    public function estadisticaEnvio($id_envio,$empresa)
    {
        ...
        $datos = array(
            ...
            'malos' => $this->malosEntregaPorEmpresa($id_envio,$empresa),
    ...
    {  
        ...
        if (!empty($estado)) {     
            if ($estado == 'confirmados') {
                ...
                $sql .= " AND D.estado in ('INVALID_DNS')";  
            } elseif ($estado == 'error') {
                $sql .= " AND D.estado in ('UNDELIVERED', 'UNKNOWN')";  
            }   
        }
   ...
   public function decodMensaje($id_envio,$id_contacto,$destinatario)
    ...
    public function errorEntregaPorEmpresa($id_envio, $empresa_telefono)
    {
        $sql = "SELECT count(id_detalle_envio) as errorEntrega
                from detalle_envio 
                where estado in ( 'UNDELIVERED','UNKNOWN')
                and id_envio = " . $id_envio ."
                and empresa_telefono_receptor = '" . $empresa_telefono . "'";
    ...
    public function malosEntregaPorEmpresa($id_envio, $empresa_telefono)
    {
        $sql = "SELECT count(id_detalle_envio) as malosEntrega
                from detalle_envio 
                where estado in ('INVALID_DNS')
                and id_envio = " . $id_envio ."
                and empresa_telefono_receptor = '" . $empresa_telefono . "'";

        $consulta = $this->admDB->query($sql);
        $result = $consulta->row['malosEntrega'];
    ...
    {
        $sql = "SELECT COUNT(*) as malos from detalle_envio where estado in ('INVALID_DNS') AND id_envio = " . $id_envio;
        $result = $this->admDB->query($sql);
    ...
    {
        $sql = "SELECT count(id_detalle_envio) as error from detalle_envio where estado in ( 'UNDELIVERED', 'UNKNOWN') and id_envio = " . $id_envio;
        $result = $this->admDB->query($sql);
