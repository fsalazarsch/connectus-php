...
class ControllerSmsListaSmsPredefinidos extends Controller {
    ...
    protected function getList() {
        ...
        $cantidad_listas = $this->model_sms_lista_sms_predefinidos->getCountSmsPredefinidos($this->session->data['id_empresa']);
        $data['lista_preferidos'] = '';
        $result_preferidos = $this->model_sms_lista_sms_predefinidos->getSmsPredefinidos($this->session->data['id_empresa'], $filter_data);
        ...
        $data['button_exportar_excel']  = $this->language->get('excel');
        $data['button_exportar_pdf']    = $this->language->get('pdf');

        $data['button_add'] = $this->language->get('text_add');

        $data['delete'] = $this->url->link('sms/lista_sms_predefinidos/delete', 'token=' . $this->session->data['token'], 'SSL');
        $data['add'] = $this->url->link('sms/crear_editar_predefinido', 'token=' . $this->session->data['token'], 'SSL');
        $data['full_excel'] = $this->url->link('sms/lista_sms_predefinidos/exp', 'token=' . $this->session->data['token'] . "&exp=e", 'SSL');
        $data['full_pdf'] = $this->url->link('sms/lista_sms_predefinidos/exp', 'token=' . $this->session->data['token'] . "&exp=p", 'SSL');
        ...
        $data['sort_nombre'] = $this->url->link('sms/lista_sms_predefinidos', 'token=' . $this->session->data['token'] . '&sort=titulo' . $url, 'SSL');
        $data['sort_fecha_creacion'] = $this->url->link('sms/lista_sms_predefinidos', 'token=' . $this->session->data['token'] . '&sort=fecha_creacion' . $url, 'SSL');                
        $data['sort_autor'] = $this->url->link('sms/lista_sms_predefinidos', 'token=' . $this->session->data['token'] . '&sort=autor' . $url, 'SSL'); 
        ...
        $pagination->url = $this->url->link('sms/lista_sms_predefinidos', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
        ...
        $this->response->setOutput($this->load->view('sms/lista_sms_predefinidos.tpl', $data));
    }
    ...
    public function delete() {
        ...
        if (isset($this->request->post['selected'])) {
            ...
            foreach ($this->request->post['selected'] as $id_mensaje) {
                $this->model_sms_lista_sms_predefinidos->deleteMensaje($id_mensaje);            
            }
            ...
            $this->response->redirect($this->url->link('sms/lista_sms_predefinidos', 'token=' . $this->session->data['token'] . $url, 'SSL'));
        }
    ...
    public function exp(){
        ...
        if($tipo!=''){
            if($tipo == 'e'){
                ...
                $file_excel = $this->model_contactos_listas->exportar_mensaje_x($id_empresa);
                $this->response->redirect($file_excel);
                $this->getList();
            }elseif ($tipo == 'p') {
                $this->model_contactos_listas->exportar_sms_predef_pdf();
            }
        }
    }
}