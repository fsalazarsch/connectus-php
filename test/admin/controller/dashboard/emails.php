<?php
class ControllerDashboardEmails extends Controller {
	public function index() {
		$this->load->language('dashboard/emails');
		...
		//$total_envios = $this->model_dashboard_client_dashboard_client->getTotalMail($this->session->data['id_empresa']);

		$this->load->model('cuenta_corriente/cuenta_corriente');
		
		//total historico de consumidos
		$total_envios = $this->model_cuenta_corriente_cuenta_corriente->creditosConsumidosMail($this->session->data['id_empresa']);

		//$total_envios = $this->model_cuenta_corriente_cuenta_corriente->creditosMailRestantes($this->session->data['id_empresa']);
		//$data['total_creditos'] = $this->model_dashboard_client_dashboard_client->getSaldoEnviosMail($this->session->data['id_empresa']);
		$total_creditos = $this->model_cuenta_corriente_cuenta_corriente->creditosMailRestantes($this->session->data['id_empresa']);
		//$data['total_creditos'] = $this->model_cuenta_corriente_cuenta_corriente->creditosMailRestantes($this->session->data['id_empresa']);
		...
		*/
		...
		//$data['comprar'] = $this->url->frontend('checkout/cart');

		return $this->load->view('dashboard/emails.tpl', $data);
	}
}