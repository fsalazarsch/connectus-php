...
class ControllerCommonHeader extends Controller {
	public function index() {
		...
		if (isset($this->session->data['tipo_usuario'])) {
		...
		//Cambia home cuando es cliente
		$home = 'common/dashboard';
		if (isset($this->request->get['token'])) {
			if ($this->session->data['tipo_usuario'] == 'cliente') {
				$home = 'common/dashboard_client';			
					
				$this->load->model('user/user');
				$user_id = $this->model_user_user->getUser($this->user->getId());
				$data['image'] = $user_id['code'];
		...
		if (!isset($this->request->get['token']) || !isset($this->session->data['token']) || ($this->request->get['token'] != $this->session->data['token'])) {
			...
			$data['home'] = $this->url->link($home, '', 'SSL');
		} else {
			$data['logged'] = true;

			$data['home'] = $this->url->link($home, 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['ayuda'] = $this->video_de_ayuda();

		return $this->load->view('common/header.tpl', $data);
	}

	public function video_de_ayuda()
	{
		$this->load->model('design/youtube');

		$route = isset($this->request->get['route']) ? $this->request->get['route'] : '';

		$ayuda = $this->model_design_youtube->link($route);
