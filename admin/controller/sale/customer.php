...
class ControllerSaleCustomer extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$sql = $this->model_sale_customer->addCustomer($this->request->post);
	...
	public function edit() {
		...
		$this->session->data['id_empresa'] = $this->request->get['customer_id'];

		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {

			$sql = $this->model_sale_customer->editCustomer($this->request->get['customer_id'], $this->request->post);
			...
			if (isset($this->request->get['customer_id'])) {
				...
				$cuentco = $this->model_cuenta_corriente_cuenta_corriente->getCuentaEmpresa($this->request->get['customer_id']);

				$info_Creditos = $this->info_Creditos();

				$sms_restantes  = $cuentco['saldo_sms'] - $cuentco['consumidos_sms'];
				$mail_restantes = $cuentco['saldo_mail'] - $cuentco['consumidos_mail'];

				$mail_aux = $this->request->post['mailrec']!=''?$this->request->post['mailrec']:0;
				$sms_aux  = $this->request->post['smsrec']!=''?$this->request->post['smsrec']:0;

				$id_empresa  = $this->request->get['customer_id'];
				$saldo_carga = ($mail_aux + $sms_aux);

				$this->load->model('cuenta_corriente/cuenta_corriente');
				$msg = $this->model_cuenta_corriente_cuenta_corriente->updateCuenta($id_empresa, $saldo_carga, $mail_aux, $sms_aux );
			}

			if ($msg) {
				$this->session->data['success'] = $this->language->get('update_empresa');
			}
			...
			//$this->response->redirect($this->url->link('sale/customer', 'token=' . $this->session->data['token'] . $url, 'SSL'));
			$this->response->redirect($this->url->link('sale/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $id_empresa, 'SSL'));
		}
	...
	public function delete() {
		...
		if (isset($this->session->data['id_empresa'])) {
			$actualmente = $this->session->data['id_empresa'];
		}else{
			$actualmente = -1;
		}
	...
	protected function getList() {
		...
		$data['button_edit_driver'] = $this->language->get('button_edit_driver');
	...
	protected function getForm() {
		...
		$data['tipo'] = $this->session->data['tipo_usuario'];
		...
		if (isset($this->request->get['customer_id'])) {
			...
			$data['tmail'] = $cuentco['saldo_mail'];
			$data['tsms']  = $cuentco['saldo_sms'];

			$info_Creditos = $this->info_Creditos();

			$data['sms_rest']  = $cuentco['saldo_sms'] - $cuentco['consumidos_sms'];
			$data['mail_rest'] = $cuentco['saldo_mail'] - $cuentco['consumidos_mail'];				
		}else{
			$data['tmail'] = 0;
			$data['tsms']  = 0;
			$data['sms_rest']  = 0;
		...
		// se agrega dirección
		$data['text_direccion'] = $this->language->get('text_direccion');
		$data['entry_direccion'] = $this->language->get('entry_direccion');

		$data['text_comuna'] = $this->language->get('text_comuna');
		$data['entry_comuna'] = $this->language->get('entry_comuna');

		$data['text_ciudad'] = $this->language->get('text_ciudad');
		$data['entry_ciudad'] = $this->language->get('entry_ciudad');
		...
		if (isset($this->error['direccion'])) {
			$data['error_direccion'] = $this->error['direccion'];
		} else {
			$data['error_direccion'] = '';
		}

		if (isset($this->error['comuna'])) {
			$data['error_comuna'] = $this->error['comuna'];
		} else {
			$data['error_comuna'] = '';
		}

		if (isset($this->error['ciudad'])) {
			$data['error_ciudad'] = $this->error['ciudad'];
		} else {
			$data['error_ciudad'] = '';
		}
		...
		$data['breadcrumbs'] = array();
			...
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('sale/customer/edit', 'token=' . $this->session->data['token'] . "&customer_id=" . $this->session->data['id_empresa'], 'SSL')
			);
		}
		...
		if (isset($this->request->post['firstname'])) {
			...
			$data['firstname'] = $customer_info['razon_social'];
		} else {
			$data['firstname'] = '';
		}
		...
		if (isset($this->request->post['lastname'])) {
			...
			$data['lastname'] = $customer_info['rut_empresa'];
		} else {
			$data['lastname'] = '';
		}
		...
		if (isset($this->request->post['direccion'])) {
			$data['direccion'] = $this->request->post['direccion'];
		} elseif (!empty($customer_info)) {
			$data['direccion'] = $customer_info['address_1'];
		} else {
			$data['direccion'] = '';
		}

		if (isset($this->request->post['comuna'])) {
			$data['comuna'] = $this->request->post['comuna'];
		} elseif (!empty($customer_info)) {
			$data['comuna'] = $customer_info['address_2'];
		} else {
			$data['comuna'] = '';
		}
		...
		if (isset($this->request->post['ciudad'])) {
			...
			$data['ciudad'] = $customer_info['city'];
		} else {
			$data['ciudad'] = '';
		}
		...
		//
		if (isset($this->request->post['input-mail'])) {
			$data['mailr'] = $this->request->post['input-mail'];
		} else {
			$data['mailr'] = '';
		}

		if (isset($this->request->post['input-sms'])) {
			$data['smsr'] = $this->request->post['input-sms'];
		} else {
			$data['smsr'] = '';
		}
	...
	public function info_Creditos(){
		$this->load->model('dashboard_client/dashboard_client');
		$creditos = array();

		$creditos['sms']  = $this->model_dashboard_client_dashboard_client->getTotalSms($this->session->data['id_empresa']);
		$creditos['mail'] = $this->model_dashboard_client_dashboard_client->getTotalMail($this->session->data['id_empresa']);

		return $creditos;
	}

	public function modoEmpresa() {
		$token = $this->request->get['token'];

		unset($this->session->data['id_empresa']);

		$this->session->data['como_empresa']  = true; 
		$this->session->data['id_empresa']    = $this->request->get['customer_id'];

		$this->response->redirect($this->url->link('common/dashboard_client', '&token=' . $token));
	}
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['firstname']) < 1) || (utf8_strlen(trim($this->request->post['firstname'])) > 200)) {
			$this->error['firstname'] = $this->language->get('error_firstname');
		}

		if ((utf8_strlen($this->request->post['lastname']) < 1) || (utf8_strlen(trim($this->request->post['lastname'])) > 200)) {
