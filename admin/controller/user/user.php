...
class ControllerUserUser extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			//id_empresa agregada para lograr un control de usuario, session es llenada en el login.
			$add_user_empresa = $this->request->post['sel_empresa'];
			$this->model_user_user->addUser($this->request->post, $add_user_empresa);
	...
	protected function getList() {
		...
		/*	lINK BREADCRUMB POR TIPO DE USUARIO	*/
		...
		if ($this->session->data['tipo_usuario'] == 'administrador') {
			...
			foreach ($results as $result) {
				if (null != $this->model_user_user->traerEmpresa($result['id_empresa']))  {
					$empresa = $this->model_user_user->traerEmpresa($result['id_empresa']);
				}else{
					$empresa = 'Sin Empresa';
				}
			...
			$user_total = $this->model_user_user->getTotalUsers($filter_data, $this->session->data['id_empresa']);
			$results = $this->model_user_user->getUsers($filter_data, $this->session->data['id_empresa']);
		...
		$data['column_empresa']  = $this->language->get('column_empresa');
		...
		//$data['sort_empresa'] = $this->url->link('user/user', 'token=' . $this->session->data['token'] . '&sort=empresa' . $url, 'SSL');
	...
	protected function getForm() {
		...
		if ($this->session->data['tipo_usuario'] == 'administrador') {
			$data['tipo_user'] = $this->session->data['tipo_usuario'];
				$data['breadcrumbs'][] = array(
					'text' => $this->language->get('text_home'),
					'href' => $this->url->link('common/dashboard', 'token=' . $this->session->data['token'], 'SSL')
				);
...
/*
		if (isset($this->session->data['id_empresa'])) {
			$data['emp'] = $this->session->data['id_empresa'];
		}else{
			$data['emp'] = 0;
		}
*/
		if (isset($this->request->get['user_id'])) {
			...
			$data['emp'] = $this->model_sale_customer->get_empresa_por_user($this->request->get['user_id']);
		}else{
			$data['emp'] = 0;
		}
		...
		$this->load->model('sale/customer');
		$data['empresas'] = $this->model_sale_customer->getCustomers();
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['username']) < 3) || (utf8_strlen($this->request->post['username']) > 70)) {
