...
class ControllerContactosPreviewLista extends Controller {
	...
	public function getList() {
        ...
        if (isset($this->error['error_campos'])) {
            //$data['error_warning_campo'] = $this->language->get('error_faltan_campos');
            $data['error_warning_campo'] = $this->error['error_campos'];
        } else {
            $data['error_warning_campo'] = '';
        }
	...
	public function guardarListaXpress(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			try {
					...
					//if ( $this->rowValidate($columna) ) {
						if($row_index < count($columna) && $col_index > 0 ){
							...
							if (isset($campos[$row_index])) {
								...
								$this->error['error_campos'] = 'Problemas en el formato del archivo: Línea '.($col_index).'. Favor corrija el archivo e intente nuevamente.';
			    				$this->getList();
			    				return;
							}
				...
				$this->error['error_campos'] = 'El archivo tiene problemas de formato. Favor corrija el archivo e intente nuevamente. Error:'.$e->getMessage();
				$this->getList();
			}
